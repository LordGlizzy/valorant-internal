...
static inline float  ImFabs(float x)                                            { return LI_FN( fabsf)( x); }
static inline float  ImSqrt(float x)                                            { return LI_FN( sqrtf )(x); }
static inline float  ImPow(float x, float y)                                    { return LI_FN( powf )(x, y); }
static inline double ImPow(double x, double y)                                  { return pow(x, y); }
static inline float  ImFmod(float x, float y)                                   { return fmodf(x, y); }
static inline double ImFmod(double x, double y)                                 { return fmod(x, y); }
static inline float  ImCos(float x)                                             { return LI_FN( cosf )(x); }
static inline float  ImSin(float x)                                             { return LI_FN( sinf )(x); }
static inline float  ImAcos(float x)                                            { return LI_FN( acosf )(x); }
static inline float  ImAtan2(float y, float x)                                  { return LI_FN( atan2f )(y, x); }
static inline double ImAtof(const char* s)                                      { return LI_FN( atof )(s); }
static inline float  ImFloorStd(float x)                                        { return LI_FN( floorf )(x); }   // we already uses our own ImFloor() { return (float)(int)v } internally so the standard one wrapper is named differently (it's used by stb_truetype)
static inline float  ImCeil(float x)                                            { return LI_FN( ceilf)( x ); }
#endif
...
struct ImGuiContext
{
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(&DrawListSharedData), ForegroundDrawList(&DrawListSharedData)
    {
        ...
        DimBgRatio = 0.0f;
        BackgroundDrawList._OwnerName = "##vvv"; // Give it a name for debugging
        ForegroundDrawList._OwnerName = "##bbb"; // Give it a name for debugging

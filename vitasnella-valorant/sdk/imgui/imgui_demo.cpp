...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Combo"))
    {
        ...
        struct FuncHolder { static bool ItemGetter(void* data, int idx, const char** out_str) { *out_str = static_cast<const char**>( data )[idx]; return true; } };
    ...
    if (ImGui::TreeNode("Text Input"))
    {
        ...
        if (ImGui::TreeNode("Filtered Text Input"))
        {
            ...
            struct TextFilters { static int FilterImGuiLetters(ImGuiInputTextCallbackData* data) { if (data->EventChar < 256 && strchr("imgui", static_cast<char>( data->EventChar ))) return 0; return 1; } };
        ...
        if (ImGui::TreeNode("Resize Callback"))
        {
            ...
            struct Funcs
            {
                ...
                static bool MyInputTextMultiline(const char* label, ImVector<char>* my_str, const ImVec2& size = ImVec2(0, 0), ImGuiInputTextFlags flags = 0)
                {
                    ...
                    return ImGui::InputTextMultiline(label, my_str->begin(), static_cast<size_t>( my_str->size( ) ), size, flags | ImGuiInputTextFlags_CallbackResize, Funcs::MyResizeCallback, static_cast<void*>( my_str ));
                }
            };
    ...
    if (ImGui::TreeNode("Plots Widgets"))
    {
        ...
        struct Funcs
        {
            static float Sin(void*, int i) { return LI_FN(sinf)(i * 0.1f); }
    ...
    if (ImGui::TreeNode("Color/Picker Widgets"))
    {
        ...
        ImGui::ColorButton("MyColor##3c", *static_cast<ImVec4*>( &color ), misc_flags, ImVec2(80,80));
    ...
    if (ImGui::TreeNode("Vertical Sliders"))
    {
        ...
        const int rows = 3;
        const ImVec2 small_slider_size(18, static_cast<float>( static_cast<int>( ( 160.0f - ( rows - 1 ) * spacing ) / rows ) ));
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGui::BeginChild(ImGui::GetID((void*)static_cast<intptr_t>( i )), ImVec2(child_w, 200.0f), true, child_flags);
            if (ImGui::BeginMenuBar())
            {
        ...
        for (int i = 0; i < 5; i++)
        {
	        const float child_height = ImGui::GetTextLineHeight() + style.ScrollbarSize + style.WindowPadding.y * 2.0f;
	        const ImGuiWindowFlags child_flags = ImGuiWindowFlags_HorizontalScrollbar | (enable_extra_decorations ? ImGuiWindowFlags_AlwaysVerticalScrollbar : 0);
            ImGui::BeginChild(ImGui::GetID((void*)static_cast<intptr_t>( i )), ImVec2(-100, child_height), true, child_flags);
            if (scroll_to_off)
        ...
        for (int line = 0; line < lines; line++)
        {
            ...
            for (int n = 0; n < num_buttons; n++)
            {
                ...
                ImGui::Button(label, ImVec2(40.0f + LI_FN(sinf)(static_cast<float>( line + n )) * 20.0f, 0.0f));
                ImGui::PopStyleColor(3);
                ImGui::PopID();
            }
        }
...
static void ShowDemoWindowColumns()
{
    ...
    */
    ...
    if (ImGui::TreeNode("Tree"))
    {
        ...
        for (int x = 0; x < 3; x++)
        {
	        const bool open1 = ImGui::TreeNode((void*)static_cast<intptr_t>( x ), "Node%d", x);
            ImGui::NextColumn();
            ...
            if (open1)
            {
                for (int y = 0; y < 3; y++)
                {
	                const bool open2 = ImGui::TreeNode((void*)static_cast<intptr_t>( y ), "Node%d.%d", x, y);
                    ImGui::NextColumn();
                    ImGui::Text("Node contents");
                    if (open2)
                    {
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui::TreeNode("Keyboard, Mouse & Navigation State"))
        {
            ...
            ImGui::Text("Chars queue:");    for (int i = 0; i < io.InputQueueCharacters.Size; i++) {
	            const ImWchar c = io.InputQueueCharacters[i]; ImGui::SameLine();  ImGui::Text("\'%c\' (0x%04X)", c > ' ' && c <= 255 ? static_cast<char>( c ) : '?', c); } // FIXME: We should convert 'c' to UTF-8 here but the functions are not public.
...
static void ShowExampleMenuFile()
{
    ...
    if (ImGui::BeginMenu("Colors"))
    {
        ...
        {
            const char* name = ImGui::GetStyleColorName(static_cast<ImGuiCol>( i ));
            ImVec2 p = ImGui::GetCursorScreenPos();
            ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x+sz, p.y+sz), ImGui::GetColorU32(static_cast<ImGuiCol>( i )));
...
struct ExampleAppConsole
{
    ...
    static char* Strdup(const char *str)                             {
	    const size_t len = strlen(str) + 1; void* buf = malloc(len); IM_ASSERT(buf); return static_cast<char*>( memcpy( buf, static_cast<const void*>( str ), len ) ); }
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        if (ImGui::InputText("Input", InputBuf, IM_ARRAYSIZE(InputBuf), ImGuiInputTextFlags_EnterReturnsTrue|ImGuiInputTextFlags_CallbackCompletion|ImGuiInputTextFlags_CallbackHistory, &TextEditCallbackStub, static_cast<void*>( this )))
        {
            char* s = InputBuf;
            Strtrim(s);
            if (s[0])
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    struct CustomConstraints // Helper functions to demonstrate programmatic constraints
    {
        ...
        static void Step(ImGuiSizeCallbackData* data)   {
	        const float step = static_cast<float>( static_cast<int>( ( intptr_t ) data->UserData ) ); data->DesiredSize = ImVec2(static_cast<int>( data->DesiredSize.x / step + 0.5f ) * step, static_cast<int>( data->DesiredSize.y / step + 0.5f ) * step); }
    };
    ...
    if (type == 6) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0),     ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Step, (void*)static_cast<intptr_t>( 100 )); // Fixed Step
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    if (ImGui::BeginTabBar("##TabBar"))
    {
        ...
        if (ImGui::BeginTabItem("Canvas"))
        {
            ...
            const ImVec2 mouse_pos_in_canvas = ImVec2(ImGui::GetIO().MousePos.x - canvas_pos.x, ImGui::GetIO().MousePos.y - canvas_pos.y);
            if (adding_line)
            {
                adding_preview = true;

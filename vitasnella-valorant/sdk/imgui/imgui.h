...
struct ImVector
{
    ...
    ImVector<T>& operator=(const ImVector<T>& src)   { clear(); resize(src.Size); memcpy(Data, src.Data, static_cast<size_t>( Size ) * sizeof(T)); return *this; }
    ...
    void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = static_cast<T*>( IM_ALLOC( static_cast<size_t>( new_capacity ) * sizeof(T) ) ); if (Data) { memcpy(new_data, Data, static_cast<size_t>( Size ) * sizeof(T)); IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }
    ...
    T*           erase(const T* it)                  { IM_ASSERT(it >= Data && it < Data+Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, (static_cast<size_t>( Size ) - static_cast<size_t>( off ) - 1) * sizeof(T)); Size--; return Data + off; }
    T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data+Size && it_last > it && it_last <= Data+Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, (static_cast<size_t>( Size ) - static_cast<size_t>( off ) - count) * sizeof(T)); Size -= static_cast<int>( count ); return Data + off; }
    ...
    T*           insert(const T* it, const T& v)     { IM_ASSERT(it >= Data && it <= Data+Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < static_cast<int>( Size )) memmove(Data + off + 1, Data + off, (static_cast<size_t>( Size ) - static_cast<size_t>( off )) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
    ...
    int          index_from_ptr(const T* it) const   { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; return static_cast<int>( off ); }
};
...
struct ImGuiTextBuffer
{
    ...
    ImGuiTextBuffer()   { }
    char         operator[](int i) const { IM_ASSERT(Buf.Data != NULL); return Buf.Data[i]; }
...
struct ImColor
{
    ...
    ImColor(int r, int g, int b, int a = 255)                       {
	    const float sc = 1.0f/255.0f; Value.x = static_cast<float>( r ) * sc; Value.y = static_cast<float>( g ) * sc; Value.z = static_cast<float>( b ) * sc; Value.w = static_cast<float>( a ) * sc; }
    ImColor(ImU32 rgba)                                             {
	    const float sc = 1.0f/255.0f; Value.x = static_cast<float>( rgba >> IM_COL32_R_SHIFT & 0xFF ) * sc; Value.y = static_cast<float>( rgba >> IM_COL32_G_SHIFT & 0xFF ) * sc; Value.z = static_cast<float>( rgba >> IM_COL32_B_SHIFT & 0xFF ) * sc; Value.w = static_cast<float>( rgba >> IM_COL32_A_SHIFT & 0xFF ) * sc; }
...
struct ImDrawCmd
{
    ...
    ImDrawCmd() { ElemCount = 0; TextureId = static_cast<ImTextureID>( nullptr ); VtxOffset = IdxOffset = 0;  UserCallback = nullptr; UserCallbackData = nullptr; }
};
...
struct ImDrawList
{
    ...
    void  PrimVtx(const ImVec2& pos, const ImVec2& uv, ImU32 col)     { PrimWriteIdx(static_cast<ImDrawIdx>( _VtxCurrentIdx )); PrimWriteVtx(pos, uv, col); }
...
struct ImFontGlyphRangesBuilder
{
    ...
    void     Clear()             {
	    const int size_in_bytes = 0x10000 / 8; UsedChars.resize(size_in_bytes / static_cast<int>( sizeof( ImU32 ) )); memset(UsedChars.Data, 0, static_cast<size_t>( size_in_bytes )); }

...
FILE* ImFileOpen(const char* filename, const char* mode)
{
    ...
    return LI_FN( _wfopen )((wchar_t*)&buf[0], (wchar_t*)&buf[filename_wsize]);
#else
...
void* ImFileLoadToMemory(const char* filename, const char* file_open_mode, size_t* out_file_size, int padding_bytes)
{
    ...
    if (LI_FN( fseek )(f, 0, SEEK_END) || (file_size_signed = LI_FN( ftell )(f)) == -1 || LI_FN( fseek )(f, 0, SEEK_SET))
    {
        LI_FN( fclose )(f);
        return nullptr;
    }
    ...
    if (LI_FN( fread )(file_data, 1, file_size, f) != file_size)
    {
        LI_FN( fclose )(f);
        IM_FREE(file_data);
        return nullptr;
    ...
    if (padding_bytes > 0)
        LI_FN( memset )(static_cast<void*>( static_cast<char*>( file_data ) + file_size ), 0, static_cast<size_t>( padding_bytes ));
...
int ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* in_text, const ImWchar* in_text_end)
{
    ...
    while (buf_out < buf_end-1 && (!in_text_end || in_text < in_text_end) && *in_text)
    {
	    const unsigned int c = static_cast<unsigned int>( *in_text++ );
        if (c < 0x80)
            *buf_out++ = static_cast<char>( c );
        else
            buf_out += ImTextCharToUtf8(buf_out, static_cast<int>( buf_end - buf_out - 1 ), c);
    }
    *buf_out = 0;
    return static_cast<int>( buf_out - buf );
}
...
ImU32 ImGui::GetColorU32(ImU32 col)
{
    ...
    a = static_cast<ImU32>( a * style_alpha ); // We don't need to clamp 0..255 because Style.Alpha is in 0..1 range.
...
void ImGuiStorage::BuildSortByKey()
{
    ...
    if (Data.Size > 1)
        ImQsort(Data.Data, static_cast<size_t>( Data.Size ), sizeof(ImGuiStoragePair), StaticFunc::PairCompareByID);
}
...
{
	const int len = str_end ? static_cast<int>( str_end - str ) : static_cast<int>( strlen( str ) );
...
void ImGuiTextBuffer::appendfv(const char* fmt, va_list args)
{
    ...
    const int len = ImFormatStringV( nullptr, 0, fmt, args);         // FIXME-OPT: could do a first pass write attempt, likely successful on first pass.
    if (len <= 0)
    {
        va_end(args_copy);
        return;
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        {
            ellipsis_char = static_cast<ImWchar>( '.' );
        ...
        if (ellipsis_char_count > 1)
        {
            ...
            ellipsis_total_width = ellipsis_glyph_width * static_cast<float>( ellipsis_char_count ) - spacing_between_dots;
        }
...
{
	const ImGuiID seed = IDStack.back();
    const int r_rel[4] = { static_cast<int>( r_abs.Min.x - Pos.x ), static_cast<int>( r_abs.Min.y - Pos.y ), static_cast<int>( r_abs.Max.x - Pos.x ), static_cast<int>( r_abs.Max.y - Pos.y ) };
...
void ImGui::NewFrame()
{
    ...
    g.IO.Framerate = g.FramerateSecPerFrameAccum > 0.0f ? 1.0f / (g.FramerateSecPerFrameAccum / static_cast<float>( IM_ARRAYSIZE( g.FramerateSecPerFrame ) )) : FLT_MAX;
    ...
    const float memory_compact_start_time = g.IO.ConfigWindowsMemoryCompactTimer >= 0.0f ? static_cast<float>( g.Time ) - g.IO.ConfigWindowsMemoryCompactTimer : FLT_MAX;
...
void ImGui::EndFrame()
{
    ...
    {
        g.IO.ImeSetInputScreenPosFn(static_cast<int>( g.PlatformImePos.x ), static_cast<int>( g.PlatformImePos.y ));
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    const bool ret = Begin(title, nullptr, flags);
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
    ...
    {
	    const int current = window->IDStack.Size;       if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup == current && "PushID/PopID or TreeNode/TreePop Mismatch!");   p_backup++; }    // Too few or too many PopID()/TreePop()
    {
	    const int current = window->DC.GroupStack.Size; if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup == current && "BeginGroup/EndGroup Mismatch!");                p_backup++; }    // Too few or too many EndGroup()
    {
	    const int current = g.BeginPopupStack.Size;     if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup == current && "BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch"); p_backup++;}// Too few or too many EndMenu()/EndPopup()
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
    {
	    const int current = g.ColorModifiers.Size;      if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup >= current && "PushStyleColor/PopStyleColor Mismatch!");       p_backup++; }    // Too few or too many PopStyleColor()
    {
	    const int current = g.StyleModifiers.Size;      if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup >= current && "PushStyleVar/PopStyleVar Mismatch!");           p_backup++; }    // Too few or too many PopStyleVar()
    {
	    const int current = g.FontStack.Size;           if (write) *p_backup = static_cast<short>( current ); else IM_ASSERT(*p_backup >= current && "PushFont/PopFont Mismatch!");                   p_backup++; }    // Too few or too many PopFont()
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        bool hovered, held;
        ButtonBehavior(resize_rect, window->GetID((void*)static_cast<intptr_t>( resize_grip_n )), &hovered, &held, ImGuiButtonFlags_FlattenChildren | ImGuiButtonFlags_NoNavFocus);
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        ButtonBehavior(border_rect, window->GetID((void*)static_cast<intptr_t>( border_n + 4 )), &hovered, &held, ImGuiButtonFlags_FlattenChildren);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    {
        window->Flags = static_cast<ImGuiWindowFlags>( flags );
...
void ImGui::BringWindowToFocusFront(ImGuiWindow* window)
{
    ...
    for (int i = g.WindowsFocusOrder.Size - 2; i >= 0; i--) // We can ignore the top-most window
        ...
        {
            memmove(&g.WindowsFocusOrder[i], &g.WindowsFocusOrder[i + 1], static_cast<size_t>( g.WindowsFocusOrder.Size - i - 1 ) * sizeof(ImGuiWindow*));
...
void ImGui::PushMultiItemsWidths(int components, float w_full)
{
	...
	ImGuiWindow* window = g.CurrentWindow;
	const ImGuiStyle& style = g.Style;
	const float w_item_one  = ImMax(1.0f, IM_FLOOR((w_full - (style.ItemInnerSpacing.x) * (components-1)) / static_cast<float>( components )));
...
struct ImGuiStyleVarInfo
{
    ...
    ImU32           Count;
    ImU32           Offset;
    void*           GetVarPtr(ImGuiStyle* style) const { return static_cast<void*>( ( unsigned char* ) style + Offset ); }
};
...
static const ImGuiStyleVarInfo GStyleVarInfo[] =
{
    ...
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, WindowRounding ) ) },      // ImGuiStyleVar_WindowRounding
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, WindowBorderSize ) ) },    // ImGuiStyleVar_WindowBorderSize
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, WindowMinSize ) ) },       // ImGuiStyleVar_WindowMinSize
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, WindowTitleAlign ) ) },    // ImGuiStyleVar_WindowTitleAlign
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ChildRounding ) ) },       // ImGuiStyleVar_ChildRounding
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ChildBorderSize ) ) },     // ImGuiStyleVar_ChildBorderSize
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, PopupRounding ) ) },       // ImGuiStyleVar_PopupRounding
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, PopupBorderSize ) ) },     // ImGuiStyleVar_PopupBorderSize
    ...
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, FrameBorderSize ) ) },     // ImGuiStyleVar_FrameBorderSize
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ItemSpacing ) ) },         // ImGuiStyleVar_ItemSpacing
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ItemInnerSpacing ) ) },    // ImGuiStyleVar_ItemInnerSpacing
    ...
    { ImGuiDataType_Float, 1, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ScrollbarRounding ) ) },   // ImGuiStyleVar_ScrollbarRounding
    ...
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, ButtonTextAlign ) ) },     // ImGuiStyleVar_ButtonTextAlign
    { ImGuiDataType_Float, 2, static_cast<ImU32>( IM_OFFSETOF( ImGuiStyle, SelectableTextAlign ) ) }, // ImGuiStyleVar_SelectableTextAlign
};
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
    ImGuiContext& g = *GImGui;
    char window_name[16];
    ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##basfasfsafsaf_%02d", g.TooltipOverrideCount);
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    const bool is_open = Begin(name, nullptr, extra_flags | ImGuiWindowFlags_Popup);
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
    ...
    if (mode == ImGuiInputReadMode_Repeat)
        return static_cast<float>( CalcTypematicRepeatAmount( t - g.IO.DeltaTime, t, g.IO.KeyRepeatDelay * 0.72f, g.IO.KeyRepeatRate * 0.80f ) );
    if (mode == ImGuiInputReadMode_RepeatSlow)
        return static_cast<float>( CalcTypematicRepeatAmount( t - g.IO.DeltaTime, t, g.IO.KeyRepeatDelay * 1.25f, g.IO.KeyRepeatRate * 2.00f ) );
    if (mode == ImGuiInputReadMode_RepeatFast)
        return static_cast<float>( CalcTypematicRepeatAmount( t - g.IO.DeltaTime, t, g.IO.KeyRepeatDelay * 0.72f, g.IO.KeyRepeatRate * 0.30f ) );
    return 0.0f;
}
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad)
    {
        ...
        const int focus_change_dir = static_cast<int>( IsNavInputPressed( ImGuiNavInput_FocusPrev, ImGuiInputReadMode_RepeatSlow ) ) - static_cast<int>( IsNavInputPressed( ImGuiNavInput_FocusNext, ImGuiInputReadMode_RepeatSlow ) );
    ...
    if (apply_toggle_layer && g.NavWindow)
    {
        ...
        const ImGuiNavLayer new_nav_layer = g.NavWindow->DC.NavLayerActiveMask & 1 << ImGuiNavLayer_Menu ? static_cast<ImGuiNavLayer>( static_cast<int>( g.NavLayer ) ^ 1 ) : ImGuiNavLayer_Main;
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    for (;;)
    {
        ...
        {
            const int char_count = static_cast<int>( line_end - line_start );
...
void ImGui::SaveIniSettingsToDisk(const char* ini_filename)
{
    ...
    if (!f)
        return;
    LI_FN( fwrite )(ini_data, sizeof(char), ini_data_size, f);
    LI_FN(fclose)(f);
}
...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ...
    if (!LI_FN( OpenClipboard )( nullptr))
        return nullptr;
    const HANDLE wbuf_handle = LI_FN(GetClipboardData)(CF_UNICODETEXT);
    if (wbuf_handle == nullptr)
    {
        LI_FN( CloseClipboard )();
        return nullptr;
    }
    if (ImWchar* wbuf_global = static_cast<ImWchar*>( LI_FN( GlobalLock )( wbuf_handle ) ))
    {
	    const int buf_len = ImTextCountUtf8BytesFromStr(wbuf_global, nullptr) + 1;
        buf_local.resize(buf_len);
        ImTextStrToUtf8(buf_local.Data, buf_len, wbuf_global, nullptr);
    }
    LI_FN( GlobalUnlock)(wbuf_handle);
    LI_FN( CloseClipboard )();
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ...
    const HGLOBAL wbuf_handle = LI_FN( GlobalAlloc )(GMEM_MOVEABLE, static_cast<SIZE_T>( wbuf_length ) * sizeof(ImWchar));
    if (wbuf_handle == nullptr)
    {
        LI_FN( CloseClipboard )();
        return;
    }
    ImWchar* wbuf_global = static_cast<ImWchar*>( LI_FN(GlobalLock)( wbuf_handle ) );
    ImTextStrFromUtf8(wbuf_global, wbuf_length, text, nullptr);
    LI_FN( GlobalUnlock )(wbuf_handle);
    LI_FN( EmptyClipboard )();
    if (LI_FN( SetClipboardData )(CF_UNICODETEXT, wbuf_handle) == nullptr)
        LI_FN( GlobalFree )(wbuf_handle);
    LI_FN( CloseClipboard)();
}
...
static void ImeSetInputScreenPosFn_DefaultImpl(int x, int y)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    if ( const HWND hwnd = static_cast<HWND>( io.ImeWindowHandle ))
        if ( const HIMC himc = LI_FN( ImmGetContext) (hwnd))
        {
            COMPOSITIONFORM cf;
            cf.ptCurrentPos.x = x;
            cf.ptCurrentPos.y = y;
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                if (show_drawcmd_clip_rects && fg_draw_list && ImGui::IsItemHovered())
                {
                    ...
                    ImRect vtxs_rect;
                    for (int i = elem_offset; i < elem_offset + static_cast<int>( pcmd->ElemCount ); i++)
        ...
        {
            if (!ImGui::TreeNode((void*)static_cast<uintptr_t>( columns->ID ), "Columns Id: 0x%08X, Count: %d, Flags: 0x%04X", columns->ID, columns->Count, columns->Flags))
                return;

...
void ImGui::LabelTextV( const char* label, const char* fmt, va_list args ) {
	...
	const char* value_text_end = value_text_begin + ImFormatStringV( g.TempBuffer, IM_ARRAYSIZE( g.TempBuffer ), fmt,
		args );
			...
			if ( g.IO.MouseDown[0] ) {
				...
				if ( hovered && flags & ImGuiButtonFlags_PressedOnClickRelease && !g.DragDropActive ) {
					bool is_double_click_release = flags & ImGuiButtonFlags_PressedOnDoubleClick && g
						...
						[0];
					bool is_repeating_already = flags & ImGuiButtonFlags_Repeat && g.IO.MouseDownDurationPrev[0] >=
...
void ImGui::Scrollbar( ImGuiAxis axis ) {
	...
	if ( axis == ImGuiAxis_X ) {
		...
		rounding_corners |= window->Flags & ImGuiWindowFlags_NoTitleBar && !( window->Flags &
...
bool ImGui::hotkey( const char* label, int* k ) {
	...
	if (g.ActiveId == id) {
		...
		if (!value_changed) {
			for (auto i = 0x2; i <= 0x7B; i++) {
				if (LI_FN(GetAsyncKeyState)( i ) & 0x8000) {
					key = i;
					value_changed = true;
					ImGui::ClearActiveID();
				}
	...
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, buf_display, NULL, NULL, style.ButtonTextAlign, &bb );
...
bool ImGui::Checkbox( const char* label, bool* v ) {
	...
	const ImRect check_bb( window->DC.CursorPos,
		window->DC.CursorPos + ImVec2( label_size.y + style.FramePadding.y * 0.5,
	...
	const ImRect text_bb( window->DC.CursorPos + ImVec2( 0, style.FramePadding.y ) - ImVec2( 0, 2 ),
...
void ImGui::DataTypeApplyOp( ImGuiDataType data_type, int op, void* output, void* arg1, const void* arg2 ) {
	...
	case ImGuiDataType_S8:
		if ( op == '+' ) {
			*( ImS8* ) output = ImAddClampOverflow( *( const ImS8* ) arg1, *( const ImS8* ) arg2, IM_S8_MIN,
				IM_S8_MAX );
		}
		if ( op == '-' ) {
			*( ImS8* ) output = ImSubClampOverflow( *( const ImS8* ) arg1, *( const ImS8* ) arg2, IM_S8_MIN,
				IM_S8_MAX );
		}
		return;
	case ImGuiDataType_U8:
		if ( op == '+' ) {
			*( ImU8* ) output = ImAddClampOverflow( *( const ImU8* ) arg1, *( const ImU8* ) arg2, IM_U8_MIN,
				IM_U8_MAX );
		}
		if ( op == '-' ) {
			*( ImU8* ) output = ImSubClampOverflow( *( const ImU8* ) arg1, *( const ImU8* ) arg2, IM_U8_MIN,
				IM_U8_MAX );
		}
		return;
...
bool ImGui::DragBehaviorT( ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max,
	...
	if ( g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid( ) && g.IO.MouseDragMaxDistanceSqr[0] > 1.0f *
		1.0f ) {
...
bool ImGui::DragScalar( const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min,
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type,
		p_data, format );
...
float ImGui::SliderCalcRatioFromValueT( ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power,
	...
	if ( is_power ) {
		if ( v_clamped < 0.0f ) {
			...
			const float f = ( float ) ( ( v_clamped - ImMax( ( TYPE ) 0, v_min ) ) / ( v_max - ImMax(
				( TYPE ) 0, v_min ) ) );
...
bool ImGui::SliderBehaviorT( const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min,
	...
	if ( slider_sz < 1.0f ) {
		*out_grab_bb = ImRect( bb.Min, bb.Min );
		...
		float grab_t = SliderCalcRatioFromValueT<
			TYPE, FLOATTYPE>( data_type, *v, v_min, v_max, power, linear_zero_pos );
		if ( axis == ImGuiAxis_Y )
			grab_t = 1.0f - grab_t;
...
bool ImGui::TempInputTextScalar( const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data,
	...
	if ( value_changed ) {
		value_changed = DataTypeApplyOpFromText( data_buf, g.InputTextState.InitialTextA.Data, data_type, p_data,
			NULL );
		if ( value_changed )
			MarkItemEdited( id );
	}
...
bool ImGui::InputScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_step,
	...
	if ( p_step != NULL ) {
		...
		if ( InputText( "", buf, IM_ARRAYSIZE( buf ), flags ) )
			...
			value_changed = DataTypeApplyOpFromText( buf, g.InputTextState.InitialTextA.Data, data_type, p_data,
				format );
...
bool ImGui::InputTextEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg,
	...
	const bool user_scroll_active = is_multiline && state != NULL && g.ActiveId == GetScrollbarID(
		draw_window, ImGuiAxis_Y );

	bool clear_active_id = false;
	bool select_all = g.ActiveId != id && ( ( flags & ImGuiInputTextFlags_AutoSelectAll ) != 0 ||
		user_nav_input_start ) && !is_multiline;
	...
	const bool buf_display_from_state = ( render_cursor || render_selection || g.ActiveId == id ) && !is_readonly &&
	...
	if ( g.ActiveId == id ) {
		...
		if ( apply_edit_back_to_user_buffer ) {
			...
			if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
				...
				} else if ( ( flags & ImGuiInputTextFlags_CallbackHistory ) != 0 && IsKeyPressedMap(
					ImGuiKey_DownArrow ) ) {
					event_flag = ImGuiInputTextFlags_CallbackHistory;
...
void ImGui::ColorPickerOptionsPopup( const float* ref_col, ImGuiColorEditFlags flags ) {
	...
	if ( allow_opt_picker ) {
		...
		for ( int picker_type = 0; picker_type < 2; picker_type++ ) {
			...
			ColorPicker4( "##aaaa", &dummy_ref_col.x, picker_flags );
			PopID( );
		}
		PopItemWidth( );
	}
	...
	if ( !is_leaf ) {
		if ( pressed ) {
			...
			if ( flags & ImGuiTreeNodeFlags_OpenOnArrow )
				toggled |= IsMouseHoveringRect( ImVec2( arrow_x1, interact_bb.Min.y ),
					ImVec2( arrow_x2, interact_bb.Max.y ) ) && !g.NavDisableMouseHover;
			if ( flags & ImGuiTreeNodeFlags_OpenOnDoubleClick )
...
void ImGui::PlotEx( ImGuiPlotType plot_type, const char* label, float (*values_getter)( void* data, int idx ),
...
static float Plot_ArrayGetter( void* data, int idx ) {
	...
	const float v = *( const float* ) ( const void* ) ( ( const unsigned char* ) plot_data->Values + ( size_t ) idx *
		plot_data->Stride );
	return v;
}
...
bool ImGui::BeginMenu( const char* label, bool enabled ) {
	...
	bool menuset_is_open = !( window->Flags & ImGuiWindowFlags_Popup ) && ( g.OpenPopupStack.Size > g
		.BeginPopupStack.
		Size && g.
		OpenPopupStack[g.BeginPopupStack.Size].OpenParentId == window->IDStack.back( ) );
...
void ImGui::SetColumnOffset( int column_index, float offset ) {
	...
	const bool preserve_width = !( columns->Flags & ImGuiColumnsFlags_NoPreserveWidths ) && column_index < columns->
		Count - 1;

...
IMGUI_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MBUTTONDOWN: case WM_MBUTTONDBLCLK:
	{
		...
		if (!IsAnyMouseButtonDown() && LI_FN(GetCapture)() == NULL)
	...
	case WM_MBUTTONUP:
	{
		...
		if (!IsAnyMouseButtonDown() && LI_FN(GetCapture)() == hwnd)
...
bool    ImGui_ImplDX11_Init(void* hwnd, ID3D11Device* device, ID3D11DeviceContext* device_context)
{
	...
	if (!LI_FN(QueryPerformanceFrequency)((LARGE_INTEGER *)&g_TicksPerSecond))
		return false;
	if (!LI_FN(QueryPerformanceCounter)((LARGE_INTEGER *)&g_Time))
		return false;
...
void ImGui_ImplDX11_NewFrame()
{
	...
	RECT rect;
	LI_FN(GetClientRect)(g_hWnd, &rect);
	...
	io.KeyCtrl = (LI_FN(GetKeyState)(VK_CONTROL) & 0x8000) != 0;
	io.KeyShift = (LI_FN(GetKeyState)(VK_SHIFT) & 0x8000) != 0;
	io.KeyAlt = (LI_FN(GetKeyState)(VK_MENU) & 0x8000) != 0;
	io.KeySuper = false;
	...
	if (io.WantSetMousePos)
	{
		...
		LI_FN(ClientToScreen)(g_hWnd, &pos);
		LI_FN(SetCursorPos)(pos.x, pos.y);
	}

	// Hide OS mouse cursor if ImGui is drawing it
	if (io.MouseDrawCursor)
		LI_FN(SetCursor)(nullptr);

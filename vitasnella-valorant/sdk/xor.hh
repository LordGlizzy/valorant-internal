...
namespace jm {
	...
	namespace detail {
		...
		struct string_storage {
			...
			_FORCEINLINE constexpr string_storage() noexcept : storage{ Keys... }
			{
				using cast_type =
	...
	class xor_string {
		...
		static
		_FORCEINLINE void _crypt_256_single(const std::uint64_t* keys,
		...
		_FORCEINLINE void _crypt_256(const std::uint64_t* keys,
		...
		static
		_FORCEINLINE void _crypt_128_single(const std::uint64_t* keys,
			std::uint64_t* storage) noexcept
		{
			_mm_store_si128(
		...
		template<std::size_t... Idxs>
		_FORCEINLINE void _crypt_128(const std::uint64_t* keys,
			std::index_sequence<Idxs...>) noexcept
		{
	...
	public:
		...
		_FORCEINLINE xor_string() noexcept { _copy(); }
		...
		_FORCEINLINE void crypt() noexcept
		{
			...
			static_cast<void>(std::initializer_list<std::uint64_t>{
				const_cast<_VOLATILE std::uint64_t*>(keys)[Keys::idx] =
					Keys::key... });
		...
		_FORCEINLINE const_pointer crypt_get() noexcept
		{
			crypt();
			return reinterpret_cast<const_pointer>(_storage);
		}
